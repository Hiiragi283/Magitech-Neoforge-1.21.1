                ...
                if (curiosInventory.getCurios().get("threadbound") != null) {
                    ItemStack threadbound =
                            curiosInventory
                                    ...
                                    .getStackInSlot(0);
                    double manaRatio =
                            ManaData.getCurrentMana(player, ManaUtil.ManaType.MANA)
                                    / ManaUtil.getMaxMana(player, ManaUtil.ManaType.MANA);
                    double noctisRatio =
                            ManaData.getCurrentMana(player, ManaUtil.ManaType.NOCTIS)
                                    / ManaUtil.getMaxMana(player, ManaUtil.ManaType.NOCTIS);
                    double luminisRatio =
                            ManaData.getCurrentMana(player, ManaUtil.ManaType.LUMINIS)
                                    / ManaUtil.getMaxMana(player, ManaUtil.ManaType.LUMINIS);
                    double fluxiaRatio =
                            ManaData.getCurrentMana(player, ManaUtil.ManaType.FLUXIA)
                                    / ManaUtil.getMaxMana(player, ManaUtil.ManaType.FLUXIA);
                    ...
                    guiGraphics.blit(
                            TEXTURE, x + 56 - noctisGaugeWidth, y + 6, 72, 0, noctisGaugeWidth, 7);
                    guiGraphics.blit(
                            TEXTURE,
                            x + 56 - luminisGaugeWidth,
                            y + 20,
                            72,
                            8,
                            luminisGaugeWidth,
                    ...
                    guiGraphics.blit(
                            ...
                            7);
                    guiGraphics.blit(
                            TEXTURE, x + 5, y + 64 - manaGaugeHeight, 8, 96, 8, manaGaugeHeight);

                    guiGraphics.blit(TEXTURE, x, y, 0, 0, 64, 91);

                    guiGraphics.blit(
                            TEXTURE,
                            x + 55 - noctisGaugeWidth + 1,
                            y + 3,
                            145 - noctisGaugeWidth,
                            0,
                            noctisGaugeWidth + 1,
                            2);
                    guiGraphics.blit(
                            TEXTURE,
                            x + 55 - luminisGaugeWidth + 1,
                            y + 17,
                            145 - luminisGaugeWidth,
                            8,
                            luminisGaugeWidth + 1,
                            2);
                    guiGraphics.blit(
                            TEXTURE,
                            x + 55 - fluxiaGaugeWidth + 1,
                            y + 31,
                            145 - fluxiaGaugeWidth,
                            16,
                            fluxiaGaugeWidth + 1,
                            2);
                    guiGraphics.blit(
                    ...
                    // if (manaRatio >= 1) {
                    // guiGraphics.blit(TEXTURE, guiGraphics.guiWidth() - 58,
                    // guiGraphics.guiHeight() - 54, 8, 88, 12, 11);
                    // int frame = Minecraft.getInstance().player != null ?
                    // (Minecraft.getInstance().player.tickCount / 5) % 7 : 0;
                    // guiGraphics.blit(TEXTURE, guiGraphics.guiWidth() - 56,
                    // guiGraphics.guiHeight() - 62, 0, 128 + 8 * frame, 8, 8);
                    // }

                    SpellComponent spellComponent = ComponentHelper.getSpells(threadbound);
                    if (spellComponent.selected() < spellComponent.spells().size()
                            && spellComponent.selected() >= 0) {
                        ...
                        double requiredManaRatio =
                                spell.getRequiredMana(player.level(), player, stack)
                                                .get(ManaUtil.ManaType.MANA)
                                        / ManaUtil.getMaxMana(player, ManaUtil.ManaType.MANA);
                        if (requiredManaRatio <= 1) {
                            int requiredManaGaugeHeight = (int) (requiredManaRatio * 48);
                            guiGraphics.blit(
                                    TEXTURE, x, y + 60 - requiredManaGaugeHeight, 0, 160, 16, 8);
                        }
                        ...
                        if (cooldown != null) {
                            ...
                            int rendery = y + 76;
                            RenderHelper.renderFramedText(
                                    guiGraphics, font, text, renderx, rendery, element);
                        }
                    } else {
                        threadbound.set(
