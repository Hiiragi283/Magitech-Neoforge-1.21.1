...
public class RangedEntityAttackPayLoadHandler {
    ...
    public static void handleDataOnMainS2C(
            final RangedEntityAttackPayload payload, final IPayloadContext context) {
        Level level = context.player().level();
        if (level.getEntity(payload.id()) instanceof RangedAttackMob mob
                && level.getEntity(payload.targetId()) instanceof LivingEntity target) {
