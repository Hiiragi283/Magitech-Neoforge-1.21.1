...
public class IlluminationTrait extends Trait {
    ...
    public void traitAction(
        ...
        if (result.getType() == BlockHitResult.Type.BLOCK
            ...
            level.setBlock(
                    placePos, Blocks.LIGHT.defaultBlockState().setValue(LightBlock.LEVEL, 15), 3);
            for (int i = 0; i < 20; i++) {
                level.addParticle(
                        new PowerupParticleEffect(
                                ...
                                1,
                                0),
                        placePos.getX() + player.getRandom().nextFloat(),
                        placePos.getY() + player.getRandom().nextFloat(),
                        placePos.getZ() + player.getRandom().nextFloat(),
                        0,
                        0,
                        0);
            }
