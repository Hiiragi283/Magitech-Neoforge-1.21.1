                ...
                .map(
                        spell ->
                                Component.translatable(
                                        "item.magitech.thread_page", spell.getDescription()))
                        ...
                        spell -> {
                            tooltipComponents.add(
                                    spell.getDescription()
                                            .withColor(spell.getElement().getSpellColor()));
                            Player player = ClientHelper.getPlayer();
                            if (player == null) return;
                            List<Component> componentList =
                                    spell.getTooltip(player.level(), player, stack);
                            int i = 0;
                            for (Component component : componentList) {
                                ...
                                int newCol =
                                        ColorHelper.Argb.getArgb(
                                                ColorHelper.Argb.getRed(col) / 2,
                                                ColorHelper.Argb.getGreen(col) / 2,
                                                ColorHelper.Argb.getBlue(col) / 2);
                                componentList.set(
                                        i,
                                        Component.empty()
                                                .append(
