...
/**
 * Contains math-related helper methods. This includes {@code float}-specific extensions to {@link
 * Math}, linear interpolation (lerp), and color-related methods.
 *
 * <p>Trigonometric functions defined in this class use the "sine table", a pre-calculated table of
 * {@code sin(N)} ({@code 0 <= N < pi * 2}).
 */
public class MathHelper {
    ...
    /**
     * The total number of entries in {@link MathHelper#ARCSINE_TABLE} and {@link
     * MathHelper#COSINE_OF_ARCSINE_TABLE}.
     *
     * <p>These tables have 257 elements because they store values for multiples of 1/256 from 0 to
     * 1, inclusive.
     */
     ...
     *
     * <p>When this constant is added to a double that is not too large, then the bits of the
     * result's mantissa reflect the original number times 256. Consequently, adding this constant
     * and then subtracting it rounds such doubles to the nearest multiple of 1/256.
     *
     * <p>This is used by {@link MathHelper#atan2} to produce an index into {@link
     * MathHelper#ARCSINE_TABLE} and {@link MathHelper#COSINE_OF_ARCSINE_TABLE}.
     */
    private static final double ROUNDER_256THS = Double.longBitsToDouble(4805340802404319232L);

    /**
     * Holds values of arcsin(x): {@code ARCSINE_TABLE[i]} is equal to {@code Math.arcsin(i /
     * 256.0)}.
     *
     * <p>This is used by {@link MathHelper#atan2} to approximate the inverse tangent function.
     */
    private static final double[] ARCSINE_TABLE = new double[257];

    /**
     * Holds values of cos(arcsin(x)): {@code COSINE_OF_ARCSINE_TABLE[i]} is equal to {@code
     * Math.cos(Math.arcsin(i / 256.0))}.
     *
     * <p>This is used by {@link MathHelper#atan2} to approximate the inverse tangent function.
     */
    ...
    /**
     * {@return a random, uniformly distributed integer value in {@code [min, max]}} If the range is
     * empty (i.e. {@code max < min}), it returns {@code min}.
     *
    ...
    /**
     * Clamps {@code value}, as an angle, between {@code mean - delta} and {@code mean + delta}
     * degrees.
     *
    ...
    /**
     * Steps from {@code from} degrees towards {@code to} degrees, changing the value by at most
     * {@code step} degrees.
     */
     ...
     * This is the delta value needed to lerp between {@code start} and {@code end} to get {@code
     * value}. In other words, {@code getLerpProgress(lerp(delta, start, end), start, end) ==
     * delta}.
     *
    ...
    public static boolean method_34945(Vec3 origin, Vec3 direction, AABB box) {
     ...
     *
     * @implNote This implementation transforms the arguments such that they lie in the first
     *     quadrant. If {@code y > x}, then {@code x} and {@code y} are swapped to minimize the
     *     error of the initial approximation. {@code x} and {@code y} are normalized, and an
     *     initial approximation of the result and the sine of the deviation from the true value are
     *     obtained using the {@link MathHelper#ARCSINE_TABLE} and {@link
     *     MathHelper#COSINE_OF_ARCSINE_TABLE} lookup tables. The error itself is approximated using
     *     the third-order Maclaurin series polynomial for arcsin. Finally, the implementation
     *     undoes any transformations that were performed initially.
     */
    ...
    /**
     * Finds the minimum value in {@code [min, max)} that satisfies the monotonic {@code predicate}.
     *
     * <p>The {@code predicate} must be monotonic, i.e. if for any {@code a}, {@code
     * predicate.test(a)} is {@code true}, then for all {@code b > a}, {@code predicate.test(b)}
     ...
     * @param predicate the predicate that returns {@code true} for integers greater than or equal
     *     to the value to be searched for
    ...
    /**
     * {@return linear interpolation of {@code delta} between {@code start} and {@code end}, except
     * that for any positive {@code delta} the value is positive}
     *
     * <p>Like {@link #lerp(float, int, int)}, {@code lerpPositive(0, 0, 10)} returns {@code 0}.
     * However, if the delta is {@code 0.01f}, the {@code lerp} method would return {@code 0} since
     * {@code 0.01 * 10} floored is {@code 0}. This method returns {@code 1} in this situation.
     *
    ...
    /**
     * A two-dimensional lerp between values on the 4 corners of the unit square. Arbitrary values
     * are specified for the corners and the output is interpolated between them.
     *
     ...
     */
    public static double lerp2(
            double deltaX, double deltaY, double x0y0, double x1y0, double x0y1, double x1y1) {
        return lerp(deltaY, lerp(deltaX, x0y0, x1y0), lerp(deltaX, x0y1, x1y1));
    }

    /**
     * A three-dimensional lerp between values on the 8 corners of the unit cube. Arbitrary values
     * are specified for the corners and the output is interpolated between them.
     *
    ...
    /**
     * Interpolates a point on a Catmull-Rom Spline. This spline has a property that if there are
     * two splines with arguments {@code p0, p1, p2, p3} and {@code p1, p2, p3, p4}, the resulting
     * curve will have a continuous first derivative at {@code p2}, where the two input curves
     * connect. For higher-dimensional curves, the interpolation on the curve is done
     * component-wise: for inputs {@code delta, (p0x, p0y), (p1x, p1y), (p2x, p2y), (p3x, p3y)}, the
     * output is {@code (catmullRom(delta, p0x, p1x, p2x, p3x), catmullRom(delta, p0y, p1y, p2y,
     * p3y))}.
     *
     ...
     * @see <a href=
     *     "https://en.wikipedia.org/wiki/Cubic_Hermite_spline#Catmull%E2%80%93Rom_spline">Cubic
     *     Hermite spline (Catmull–Rom spline)</a>
     */
    ...
    public static float wrap(float value, float maxDeviation) {
        return (Math.abs(value % maxDeviation - maxDeviation * 0.5F) - maxDeviation * 0.25F)
                / (maxDeviation * 0.25F);
    }
     ...
     */
    public static double clampedMap(
            double value, double oldStart, double oldEnd, double newStart, double newEnd) {
     ...
     */
    public static float clampedMap(
            float value, float oldStart, float oldEnd, float newStart, float newEnd) {
        return clampedLerp(newStart, newEnd, getLerpProgress(value, oldStart, oldEnd));
    }
     ...
     *
     * <p>For the return value {@code result}, {@code getLerpProgress(value, oldStart, oldEnd)} is
     * approximately equal to {@code getLerpProgress(result, newStart, newEnd)} (accounting for
    ...
    /**
     * Returns a value farther than or as far as {@code value} from zero that is a multiple of
     * {@code divisor}.
     */
    ...
    public static IntStream stream(int seed, int lowerBound, int upperBound, int steps) {
        if (lowerBound > upperBound) {
            ...
            throw new IllegalArgumentException(
                    String.format(Locale.ROOT, "steps expected to be >= 1, was %d", steps));
        } else {
    ...
    public static Quaternionf rotateAround(
            Vector3f axis, Quaternionf rotation, Quaternionf result) {
