...
public class CuriosHelper {
    ...
    private static @NotNull Optional<ICuriosItemHandler> getOptionalCuriosInventory(
            @Nullable LivingEntity entity) {
        return entity == null ? Optional.empty() : CuriosApi.getCuriosInventory(entity);
    }

    private static @NotNull Optional<ICurioStacksHandler> getThreadBoundSlot(
            @Nullable LivingEntity entity) {
        return getOptionalCuriosInventory(entity)
                .map(ICuriosItemHandler::getCurios)
                .flatMap(map -> Optional.ofNullable(map.get("threadbound")));
    }
