...
public class RenderHelper {
        ...
        for (int i = -2; i < 3; i++) {
            for (int j = -2; j < 3; j++) {
                ...
                guiGraphics.drawString(
                        font, text, x + (float) i / 2, y + (float) j / 2, frameColor, false);
            }
        }
        guiGraphics.drawString(font, text, x, y, color, false);
    }

    public static Component getGradationGauge(
            int min, int max, int current, int length, int startColor, int endColor) {
        ...
        for (int i = 0; i < length; i++) {
            if (i < litLength) {
                ...
                int r =
                        (int)
                                ((startColor >> 16 & 0xFF) * (1 - colorRatio)
                                        + (endColor >> 16 & 0xFF) * colorRatio);
                int g =
                        (int)
                                ((startColor >> 8 & 0xFF) * (1 - colorRatio)
                                        + (endColor >> 8 & 0xFF) * colorRatio);
                int b =
                        (int)
                                ((startColor & 0xFF) * (1 - colorRatio)
                                        + (endColor & 0xFF) * colorRatio);
    ...
    public static List<Component> getUpgradeTooltips(
            List<UpgradeInstance> instances, ToolType type, boolean isSpellCaster) {
        List<ToolStats> statsList =
                instances.stream()
                        .map(
                                i ->
                                        ToolStats.mulWithoutElementCode(
                                                i.upgrade().getUpgradeStats(i.level()),
                                                ToolMaterialRegister.getBaseStats(type)))
                        .toList();
        ...
        if (finalStats.getStats().get(ToolStats.ATK_STAT) > 0) {
            tooltipComponents.add(
                    Component.translatable(
                                    isSpellCaster
                                            ? "attribute.magitech.spell_power"
                                            : "attribute.magitech.attack_damage")
                            .withColor(0xa0a0a0)
                            .append(
                                    Component.literal(
                                                    " +"
        ...
        if (finalStats.getStats().get(ToolStats.ELM_ATK_STAT) > 0) {
            tooltipComponents.add(
                    Component.translatable(
                                    isSpellCaster
                                            ? "attribute.magitech.elemental_spell_power"
                                            : "attribute.magitech.elemental_damage")
                            .withColor(0xa0a0a0)
                            .append(
                                    Component.literal(
                                                    " +"
        ...
        if (finalStats.getStats().get(ToolStats.SPD_STAT) > 0) {
            tooltipComponents.add(
                    Component.translatable(
                                    isSpellCaster
                                            ? "attribute.magitech.casting_speed"
                                            : "attribute.magitech.attack_speed")
                            .withColor(0xa0a0a0)
                            .append(
                                    Component.literal(
                                                    " +"
        ...
        if (finalStats.getStats().get(ToolStats.MIN_STAT) > 0) {
            tooltipComponents.add(
                    Component.translatable(
                                    isSpellCaster
                                            ? "attribute.magitech.cooldown_speed"
                                            : "attribute.magitech.mining_speed")
                            .withColor(0xa0a0a0)
                            .append(
                                    Component.literal(
                                                    " +"
        ...
        if (finalStats.getStats().get(ToolStats.RNG_STAT) > 0) {
            tooltipComponents.add(
                    Component.translatable(
                                    isSpellCaster
                                            ? "attribute.magitech.projectile_speed"
                                            : "attribute.magitech.attack_range")
                            .withColor(0xa0a0a0)
                            .append(
                                    Component.literal(
                                                    " +"
        ...
        if (finalStats.getStats().get(ToolStats.SWP_STAT) > 0) {
            tooltipComponents.add(
                    Component.translatable(
                                    isSpellCaster
                                            ? "attribute.magitech.mana_efficiency"
                                            : "attribute.magitech.sweep_range")
                            .withColor(0xa0a0a0)
                            .append(
                                    Component.literal(
                                                    " +"
