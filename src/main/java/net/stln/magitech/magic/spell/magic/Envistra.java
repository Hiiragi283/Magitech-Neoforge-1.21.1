...
import net.stln.magitech.particle.RuneParticleEffect;
    ...
    public void finishUsing(
        ...
        if (livingEntity instanceof Player user) {
            if (ChargeData.getCurrentCharge(user) == null
                ...
                Vec3 hitPos =
                        EntityUtil.raycast(
                                        ...
                                        getDamage(
                                                ...
                                                (float) baseMaxRange,
                                                this.getElement()))
                                .subtract(forward.scale(livingEntity.getBbWidth() / 2 + 0.1));
                Vec3 start =
                ...
                for (int i = 0; i < hitPos.distanceTo(user.position()); i++) {
                    nearbyEntities.addAll(
                            EntityUtil.getEntitiesInBox(
                                    ...
                                    user,
                                    user.position()
                                            .lerp(hitPos, i / hitPos.distanceTo(user.position())),
                                    new Vec3(3, 3, 3)));
                }
                for (int j = 0; j < 10; j++) {
                    level.addParticle(
                            new RuneParticleEffect(new Vector3f(1), new Vector3f(1), 5F, 1, 0.3F),
                            offset.x + (user.getRandom().nextFloat() - 0.5) / 4,
                            offset.y + (user.getRandom().nextFloat() - 0.5) / 4,
                            offset.z + (user.getRandom().nextFloat() - 0.5) / 4,
                            back.x * 0.75 + (user.getRandom().nextFloat() - 0.5) / 2,
                            back.y * 0.75 + (user.getRandom().nextFloat() - 0.5) / 2,
                            back.z * 0.75 + (user.getRandom().nextFloat() - 0.5) / 2);
                }
                ...
                for (Entity entity :
                        EntityUtil.getEntitiesInBox(level, user, hitPos, new Vec3(6, 6, 6))) {
                    EffectUtil.entityEffect(
                            level,
                            new RuneParticleEffect(
                                    new Vector3f(1.0F, 1.0F, 1.0F),
                ...
                for (int i = 0; i < 60; i++) {
                    Vec3 off =
                            new Vec3(
                                    3 * (user.getRandom().nextFloat() - 0.5),
                                    3 * (user.getRandom().nextFloat() - 0.5),
                                    3 * (user.getRandom().nextFloat() - 0.5));
