...
public abstract class Spell implements SpellLike {
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, Spell> STREAM_CODEC =
            ByteBufCodecs.registry(MagitechRegistries.Keys.SPELL);
    ...
    public Map<ManaUtil.ManaType, Double> getRequiredMana(
            Level level, Player user, ItemStack stack) {
        ...
        if (level.isClientSide) {
            ...
            if (isHost) {
                PacketDistributor.sendToServer(
                        new UseSpellPayload(hand == InteractionHand.MAIN_HAND, user.getUUID()));
            }
        }
        if (canHoldUsing()) {
            user.startUsingItem(hand);
        }
        UsedHandData.setUsedHand(
                user, (hand == InteractionHand.OFF_HAND) ^ (user.getMainArm() == HumanoidArm.LEFT));
    ...
    public void usingTick(Level level, LivingEntity livingEntity, ItemStack stack, int usingTick) {
        if (livingEntity instanceof Player user) {
            InteractionHand hand =
                    user.getItemInHand(InteractionHand.MAIN_HAND) == stack
                            ? InteractionHand.MAIN_HAND
                            : InteractionHand.OFF_HAND;
            UsedHandData.setUsedHand(
                    user,
                    (hand == InteractionHand.OFF_HAND) ^ (user.getMainArm() == HumanoidArm.LEFT));
        }
    }

    public boolean isActiveUsingTick(
            Level level, LivingEntity livingEntity, ItemStack stack, int usingTick) {
        return true;
    }
    ...
    public void finishUsing(
        ...
        if (livingEntity instanceof Player player) {
            ...
            if (charge != null) {
                if (charge.getCharge() >= charge.getMaxCharge()
                        || timeCharged >= charge.getMaxCharge()) {
        ...
        if (isHost) {
            if (level.isClientSide) {
                PacketDistributor.sendToServer(
                        new ReleaseUsingSpellPayload(stack, timeCharged, livingEntity.getUUID()));
            }
        }
        if (level.isClientSide) {
    ...
    public void addCooldown(Level level, Player user, ItemStack stack) {
        CooldownData.addCurrentCooldown(
                user,
                this,
                new Cooldown(
                        this.getCooldown(level, user, stack)
    ...
    public void applyDamage(
            float baseDamage,
            Map<ManaUtil.ManaType, Double> cost,
            Element element,
            ItemStack stack,
            Player user,
            Entity target) {
        ...
        if (target.isAttackable()) {
            ...
            if (target instanceof LivingEntity livingTarget && livingTarget.invulnerableTime < 10) {
                ...
                if (!target.isInvulnerableTo(elementalDamageSource)) {
                    ...
                    user.awardStat(
                            Stats.DAMAGE_DEALT,
                            Math.round((targetHealth - livingTarget.getHealth()) * 10));
                }
    ...
    public List<Component> getTooltip(Level level, Player user, ItemStack stack) {
        ...
        list.add(
                getElement()
                        .getSpellElementName()
                        .withColor(getElement().getSpellColor())
                        .append(
                                Component.literal(" ")
                                        .append(
                                                Component.translatable(
                                                                "spell_shape.magitech."
                                                                        + getSpellShape().get())
                                                        .withColor(getSpellShape().getDark()))));
        if (this.baseDamage != 0) {
            list.add(
                    Component.translatable("tooltip.magitech.spell.damage")
                            .append(
                                    ": "
                                            + MathUtil.round(
                                                    this.getDamage(
        ...
        if (this.tickBaseDamage != 0) {
            list.add(
                    Component.translatable("tooltip.magitech.spell.tick_damage")
                            .append(
                                    ": "
                                            + MathUtil.round(
                                                    this.getDamage(
        ...
        if (this.baseSpeed != 0) {
            list.add(
                    Component.translatable("tooltip.magitech.spell.projectile_speed")
                            .append(
                                    ": "
                                            + MathUtil.round(
                                                    this.getProjectileSpeed(user, this.baseSpeed),
                                                    2)));
        }
        if (this.baseEffectStrength != 0) {
            list.add(
                    Component.translatable("tooltip.magitech.spell.effect_strength")
                            .append(
                                    ": "
                                            + MathUtil.round(
                                                    this.getDamage(
        ...
        if (this.baseDuration != 0) {
            list.add(
                    Component.translatable("tooltip.magitech.spell.duration")
                            .append(
                                    ": "
                                            + MathUtil.round(
                                                    this.getDamage(
        ...
        if (this.baseMaxRange != 0) {
            list.add(
                    Component.translatable("tooltip.magitech.spell.max_range")
                            .append(
                                    ": "
                                            + MathUtil.round(
                                                    this.getDamage(
        ...
        list.add(
                Component.translatable("tooltip.magitech.spell.cooldown")
                        .append(
                                ": "
                                        + MathUtil.round(
                                                (double)
                                                                this.getModifiedCooldown(
                                                                        level, user, stack)
                                                        / 20,
                                                2)
                                        + "s"));
        ...
        if (!Objects.equals(
                this.getBaseRequiredMana().get(ManaUtil.ManaType.MANA),
                this.getBaseCost().get(ManaUtil.ManaType.MANA))) {
            list.add(
                    Component.translatable("tooltip.magitech.spell.required_mana")
                            .append(
                                    ": "
                                            + MathUtil.round(
        ...
        if (this.needsUseCost(level, user, stack)
                && this.getBaseCost().containsKey(ManaUtil.ManaType.MANA)) {
            list.add(
                    Component.translatable("tooltip.magitech.spell.use_cost")
                            .append(
                                    ": "
                                            + MathUtil.round(
                                                    this.getCost(level, user, stack)
                                                            .get(ManaUtil.ManaType.MANA),
                                                    2))
                            .withColor(0x40FFF0));
        }
        if (this.needsTickCost(level, user, stack)
                && this.canHoldUsing()
                && this.getBaseTickCost().containsKey(ManaUtil.ManaType.MANA)) {
            list.add(
                    Component.translatable("tooltip.magitech.spell.tick_cost")
                            .append(
                                    ": "
                                            + MathUtil.round(
