...
public class Quaveris extends Spell {
    ...
    public static void breakBlockWithoutTool(Level level, BlockPos pos, Player player) {
        ...
        state.getBlock()
                .playerDestroy(level, player, pos, state, level.getBlockEntity(pos), fakeWoodTool);
        // ブロックの破壊音を再生
    ...
    public void finishUsing(
        ...
        if (livingEntity instanceof Player user) {
            if (ChargeData.getCurrentCharge(user) == null
                ...
                for (int j = 0; j < 10; j++) {
                    level.addParticle(
                            new WaveParticleEffect(new Vector3f(1), new Vector3f(1), 5F, 1, 0.3F),
                ...
                for (int i = -2; i < 2; i++) {
                    for (int j = -2; j < 2; j++) {
                        for (int k = -2; k < 2; k++) {
                            breakBlockWithoutTool(
                                    level, user.blockPosition().offset(i, j, k), user);
                        }
                    }
                }
