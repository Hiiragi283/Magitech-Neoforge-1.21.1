...
public class Arclume extends Spell {
    ...
    public void use(Level level, Player user, InteractionHand hand, boolean isHost) {
        ...
        level.playSound(
                ...
                SoundSource.PLAYERS,
                1.0F,
                1.0F + (user.getRandom().nextFloat() * 0.6F));
            ...
            TickScheduler.schedule(
                    ...
                    () -> {
                        addLightning(
                                ...
                                user,
                                level,
                                oldPos.lerp(hitPos, finalI / hitPos.subtract(oldPos).length() * 2));
                    },
                    level.isClientSide);
        }
        if (isHost) {
    ...
    private void addLightning(ItemStack stack, Player user, Level level, Vec3 pos) {
        ...
        List<Entity> entities =
                EntityUtil.getEntitiesInBox(level, user, surface, new Vec3(2, 2, 2));
        ...
        level.playSound(
                ...
                SoundSource.PLAYERS,
                1.0F,
                0.8F + (user.getRandom().nextFloat() * 0.6F));
        ...
        if (level.isClientSide) {
            level.addParticle(
                    new ZapParticleEffect(
                            new Vector3f(1), new Vector3f(1), lightningTop.toVector3f(), 2F, 3, 0),
                    surface.x,
                    surface.y,
                    surface.z,
                    0,
            ...
            for (int i = 0; i < particleAmount; i++) {
                ...
                level.addParticle(
                        new SquareParticleEffect(fromColor, toColor, scale, twinkle, rotSpeed),
                        x,
                        y,
                        z,
                        vx,
