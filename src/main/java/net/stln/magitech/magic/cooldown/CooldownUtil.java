...
public class CooldownUtil {
    ...
    public static void tick(Entity entity) {
        if (entity instanceof Player player) {
            Table<Player, Spell, Cooldown> prevData =
                    CooldownData.getPrevCooldownMap(player.level().isClientSide);
            ...
            Table<Player, Spell, Cooldown> data =
                    CooldownData.getCooldownMap(player.level().isClientSide);
            List<Spell> spellsToRemove = new ArrayList<>();
            TableHelper.forEach(
                    data,
                    (player1, spell, cooldown) -> {
